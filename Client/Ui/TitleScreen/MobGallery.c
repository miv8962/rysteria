...
struct rr_ui_element *rr_ui_mob_container_init()
{
    struct rr_ui_element *this = rr_ui_2d_container_init(
        rr_rarity_id_max, rr_mob_id_golden_meteor + 1, 16, 16);
    for (uint8_t id = 0; id <= rr_mob_id_golden_meteor; ++id)
...
struct rr_ui_element *rr_ui_mob_gallery_toggle_button_init()
{
    ...
    struct rr_ui_element *hotkey = rr_ui_text_init("[V]", 18, 0xffffffff);
    this->data = hotkey;
